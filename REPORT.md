...
The implementation is a single-process vertical slice with explicit ports instead of premature services. `DiscoveryAgent` owns the model-driven observe-decide-act loop. A `ModelProvider` supplies one structured decision at a time; the repository includes Groq Chat Completions, OpenAI Responses, Codex CLI, and an explicitly labeled scripted test provider. Both hosted providers use strict JSON-schema output, and the same local validator remains the trust boundary. `WebSurface` owns perception and interaction. `PolicyGuard` authorizes the intended action against the actual target frame URL before execution. `EvidenceRecorder` stores sanitized decisions and results, not raw model transcripts. On success, discovery returns a `CapabilityArtifact` that is independent of the provider conversation.
...
- A pre-action target-resolution failure or read-only extraction can use bounded step retry. Once a click, type, or navigation may have taken effect, a failed postcondition stops with evidence rather than risking duplicate execution. With `--handoff-on-failure`, only an extraction receives one final attempt after the operator resumes; other uncertain actions stop for review.
...
The browser is not closed or recreated. In manual mode it is already headed; automation pauses while the operator interacts with that exact page and browser context, then the operator signals resume in the terminal. An irreversible action uses a distinct prompt: the operator must explicitly type `APPROVE` for the proposed step, or it is rejected. The intervention carries a digest of the exact action rather than persisting its arguments. Browser-context listeners capture click/change/input metadata across frame navigations during the lease while redacting entered values; navigation records omit query strings. When control returns, replay records the disposition and transfer and re-evaluates manual blockers before continuing. The evidence pack demonstrates the operator clearing a verification dialog and replay extracting the output afterward in the same frame session.

A production operator console would authenticate the operator, persist leases with compare-and-swap semantics, stream the session through a secure co-browsing channel, support durable abort dispositions, and recover leases after worker failure. The current terminal signal is a minimal real handoff, not a claim to be that console.

# 6. Safety

The policy is deny-by-default along three dimensions: allowed origins, route regexes, and action kinds. Authorization uses the resolved frame's URL, preventing an allowlisted top page from smuggling interaction into an untrusted embedded origin. Playwright request interception blocks out-of-bound navigations and application requests before the browser sends them; static assets must remain on an allowed origin. Target text is checked against configurable risky patterns, and either an explicit irreversible risk or a risky target requires affirmative human approval; a `block` policy is also supported. Model suggestions are never executed before policy checks, and recovery actions pass through the same guard.

Artifacts contain parameter references instead of invocation values. Secrets remain environment-only. Observations redact configured sensitive DOM nodes plus common identifiers, amounts, email addresses, SSNs, and secret patterns before persistence. Evidence redaction applies to whole sensitive subtrees, so structured money values cannot bypass string masking. Screenshots mask inputs and configured sensitive controls. The optional OpenAI provider explicitly sends `store: false`; the Groq provider uses its direct Chat Completions endpoint. Regulated deployment still requires an approved private endpoint, tenant isolation, encryption, access controls, and a verified retention policy. Deterministic replay avoids that model disclosure entirely.
...
I deliberately did not build queues, a database, a production operator console, desktop automation, visual OCR targeting, or a tenant management service. The approval field and compatibility fingerprint are represented but there is no promotion UI, confidence scorer, or automatic drift gate. Evidence is local filesystem storage rather than encrypted object storage. The hosted providers use direct HTTPS calls to keep the dependency surface small, and the demo implements one read-oriented capability rather than many workflows.
