...
- `runs/discovery-live/` - observe/decide/act discovery events and masked success screenshot. Check `run_started.payload.provider` and artifact provenance to distinguish a genuine `codex-cli`/`openai-responses` run from the explicitly labeled `scripted-offline` fixture.
...
Regenerate offline with `npm run evidence`. Generate the required genuine model run with:
