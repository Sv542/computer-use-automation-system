...
import type { InterventionRequest } from "../domain/types.js";
...
export type Operator = (page: Page, request: InterventionRequest) => Promise<void>;
...
export class HandoffCoordinator {
  ...
  private async waitForManualResume(): Promise<void> {
    if (!input.isTTY) {
  ...
  async request(options: {
    ...
    if (this.operator) await this.operator(options.surface.page, request);
    else await this.waitForManualResume();
